...
6. Build and launch the app with `macos-build-run-debug` to verify the result in
   a real foreground `.app` bundle.
7. If SwiftUI scene/window modifiers are not enough, switch to `macos-appkit-interop`
...
- Use `macos-swiftui-patterns` for broader scene, commands, settings, sidebar,
  and inspector architecture.
- Use `macos-liquid-glass` when the main question is modern macOS visual treatment,
  Liquid Glass, or system material adoption.
- Use `macos-appkit-interop` if a custom window behavior truly requires `NSWindow`,
  `NSPanel`, or responder-chain control.
- Use `macos-build-run-debug` to launch and verify the resulting windows.
