...
When at least two providers have daily spend data (Claude, Codex, Cursor, Grok, and others that track cost), a **Total Spend** card sits above the provider sections. A capsule switcher flips between **Today**, **Yesterday**, and **30 Days** (the choice sticks across restarts), and the card shows spend over time as stacked bars — one bar per hour for Today and Yesterday, one per day for 30 Days — with each bar stacked by provider. The exact period total (to the cent) sits top-left with the share icon opposite it, and a ranked color legend (biggest spender first, with each provider's period total) closes the card. Hovering a bar shows that time slot's per-provider cost and tokens. Each provider keeps a fixed color drawn from its brand (Claude's terracotta, OpenAI's green, and so on), so colors never shift between periods. Providers with no usage for the selected period simply don't appear — they're never counted as $0 — and when any contributor's number is a local estimate the total carries a small ⓘ (hover it for the note). The share icon (or right-clicking the card) copies a branded PNG of the chart to your clipboard, just like sharing a provider card. With only one spend-tracking provider the card stays hidden — the total would just repeat that provider's own rows. The bars are fed by the same locally priced usage events as each provider's own spend rows, so the chart and the tiles always agree.
